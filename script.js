...
// ========================================================================
const sosmedData = [
  { nama: 'Instagram', sub: '@tm1_official', url: 'https://instagram.com', icon: 'camera' },
  { nama: 'TikTok', sub: '@tm1_story', url: 'https://tiktok.com', icon: 'hash' },
  { nama: 'Grup WhatsApp', sub: 'Komunitas Kelas TM-1', url: 'https://whatsapp.com', icon: 'chat' },
];

// Ikon SVG (satu library konsisten, menggantikan bekas ikon emoji)
const sosmedIconPaths = {
...
function navigateTo(page, element) {
  ...
  if (window.innerWidth <= 1024) {
    ...
    if (icon) icon.innerHTML = ICON_MENU_BARS;
  }
  window.scrollTo({ top: 0, behavior: 'smooth' });
}

const ICON_MENU_BARS = '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/>';
const ICON_MENU_CLOSE = '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>';

function toggleSidebar() {
  const sidebar = document.getElementById('sidebar');
  const isOpen = sidebar.classList.toggle('open');
  const icon = document.getElementById('mobileToggleIcon');
  if (icon) icon.innerHTML = isOpen ? ICON_MENU_CLOSE : ICON_MENU_BARS;
}
...
function renderSosmed() {
  ...
  list.innerHTML = sosmedData.map(s => `
    <a href="${s.url}" target="_blank" class="sosmed-link">
      <div class="sosmed-icon"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24">${sosmedIconPaths[s.icon] || ''}</svg></div>
      <div>
  ...
  if (activities.length === 0) {
    timeline.innerHTML = `
      <div class="empty-state">
        <div class="empty-state-icon"><svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg></div>
...
const ICON_MUSIC_NOTE = '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 18V5l12-2v13M9 18a3 3 0 11-6 0 3 3 0 016 0zm12-2a3 3 0 11-6 0 3 3 0 016 0z"/>';
const ICON_MUSIC_PAUSE = '<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 9v6m4-6v6M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/>';

function updateMusicIcon(playing) {
  if (!musicIcon || !musicToggleBtn) return;
  musicIcon.innerHTML = playing ? ICON_MUSIC_PAUSE : ICON_MUSIC_NOTE;
